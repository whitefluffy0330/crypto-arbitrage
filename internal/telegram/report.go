...
func ReportProgress(bot *tgbotapi.BotAPI, msg *tgbotapi.Message, srv *gsheets.Service, spreadsheetID string) {
	...
	if !goalExists {
		...
		if errSheet != nil {
			...
			// ВИПРАВЛЕНО: Кінцева дата для підрахунку робочих днів
			// Якщо ціль на N днів, і SetDate - це день 1, то останній день цілі = SetDate + (N-1) днів.
			...
			if currentDateForCalc.After(goalActualLastDay) {
				...
				// Передаємо currentDateForCalc як startDate, і goalActualLastDay як endDate
				actualWorkingDaysLeft, errWorkingDays = sheets.CountWorkingDaysInRange(srv, spreadsheetID, currentDateForCalc, goalActualLastDay)
			...
			reportText = fmt.Sprintf(
				...
				currentUserGoal.SetDate.In(kyivLocation).Format("02.01.2006"),
				goalActualLastDay.Format("02.01.2006"), // Додано дату завершення
