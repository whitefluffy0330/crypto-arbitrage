...
func requestAndLog(bot *tgbotapi.BotAPI, c tgbotapi.CallbackConfig, commandName string, chatID int64) { // Змінено тип на CallbackConfig
	if _, err := bot.Request(c); err != nil { // bot.Request приймає RequestConfig, який CallbackConfig реалізує
...
func HandleUpdate(bot *tgbotapi.BotAPI, update tgbotapi.Update, srv *gsheets.Service, cfg config.Config) {
	...
	if update.CallbackQuery != nil {
		...
		var callbackResponseText string // Текст для відповіді на callback (не для редагування повідомлення)
		originalMessageText := ""
		...
		case CallbackConfirmCloseGoal:
			...
			if err != nil {
				if strings.Contains(err.Error(), "не знайдено активної цілі") { callbackResponseText = "ℹ️ Активну ціль не знайдено." } else { callbackResponseText = "⚠️ Помилка закриття цілі."; log.Printf("DeleteUserGoal err: %v", err) }
		...
		requestAndLog(bot, answerCallbackCfg, "answer_callback", chatID) // Використовуємо нову requestAndLog
		return
	}
	...
	case "/goal", "🎯 Моя ціль": 
		...
		if exists { 
			...
			goalInfoText := fmt.Sprintf("📌 Ваша поточна фінансова ціль:\n\nСума: `%.2f %s`\n(Ціль на %s %d)\nВстановлено: `%s`\n\nЩоб встановити нову ціль на поточний місяць (вона перезапише поточну), надішліть її деталі у форматі `СУМА [ВАЛЮТА]`.", 
			    ...
			    monthNameUkrainian(currentGoal.SetDate.In(sheets.KyivLocation).Month()), currentGoal.SetDate.In(sheets.KyivLocation).Year(),
	...
	default:
		...
		keyboard.ShowMainKeyboard(bot, chatID) // keyboard.ShowMainKeyboard сама надсилає повідомлення
	}
}
