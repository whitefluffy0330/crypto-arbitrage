...
func HandleUpdate(bot *tgbotapi.BotAPI, update tgbotapi.Update, srv *gsheets.Service, cfg config.Config) {
	...
	case keyboard.BtnSpreads, "/spreads": 
		...
		if errInitial == nil && sentInitialMsg.MessageID != 0 { // Перевіряємо sentInitialMsg на nil
			initialMsgID = sentInitialMsg.MessageID
		} else if errInitial != nil {
			log.Printf("Спреди: Не вдалося надіслати початкове повідомлення 'Розпочато пошук...': %v", errInitial)
			// Якщо не вдалося надіслати, originalMessageID буде 0, 
			// HandleSpreadsCommand надішле фінальний звіт новим повідомленням.
		}
