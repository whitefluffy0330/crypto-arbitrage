...
import (
	...
	// "github.com/whitefluffy0330/crypto-arbitrage/internal/telegram/motivation" // Якщо не використовується
...
func HandleUpdate(bot *tgbotapi.BotAPI, update tgbotapi.Update, srv *gsheets.Service, cfg config.Config) {
	...
	case keyboard.BtnSpreads, "/spreads": 
		...
		go HandleSpreadsCommand(bot, chatID, cfg) // Запускаємо в горутині
		...
		keyboard.ShowMainKeyboard(bot, chatID) 
		return // Важливо, щоб не потрапити в default, поки горутина працює
...
} // <--- ЗАКРИВАЮЧА ДУЖКА ДЛЯ HandleUpdate
