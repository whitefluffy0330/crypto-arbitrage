...
import (
	...
	"log"
	"strconv"
	"strings" // ПОВЕРНУТО - потрібен для Split
	"time"
...
func GetCurrentTimeInKyiv() time.Time { return time.Now().In(KyivLocation) } // Можна зробити публічною, якщо треба десь ще
...
// FormatDuration публічна функція для форматування тривалості (ОДНЕ ВИЗНАЧЕННЯ)
...
func NewService(credentialsJSON []byte) (*sheets.Service, error) { return nil, fmt.Errorf("функція NewService не реалізована") } // Заглушка
...
func UpdateGoalStatusInSheet(srv *sheets.Service, spreadsheetID string, goalsSheetName string, chatID int64, newStatus string, closedDate time.Time) error {
	...
	if len(resp.Values) > 0 {
		for i := len(resp.Values) - 1; i >= 0; i-- {
			row := resp.Values[i]; isHeader := (i == 0 && len(row) > 0 && (fmt.Sprintf("%v", row[0]) == "ChatID" || fmt.Sprintf("%v", row[0]) == "ID чату користувача")); if isHeader || len(row) < 6 { continue }
	...
	if err != nil { log.Printf("Помилка оновлення статусу цілі: %v", err); return fmt.Errorf("не вдалося оновити статус: %w", err) }
	closedDateUpdateRange := fmt.Sprintf("%s!H%d", goalsSheetName, targetSheetRowIndex)
	closedDateValueRange := &sheets.ValueRange{Values: [][]interface{}{{closedDate.In(KyivLocation).Format("2006-01-02")}}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, closedDateUpdateRange, closedDateValueRange).ValueInputOption("USER_ENTERED").Do()
...
func LogWorkStart(srv *sheets.Service, spreadsheetID string, workLogSheetName string, chatID int64, startTime time.Time) error {
	if workLogSheetName == "" { workLogSheetName = workLogSheetNameDefault }; localStartTime := startTime.In(KyivLocation); todayStr := localStartTime.Format("2006-01-02"); startTimeStr := localStartTime.Format("15:04:05"); log.Printf("Лог початку роботи: ChatID=%d, Date=%s, Time=%s, Sheet=%s", chatID, todayStr, startTimeStr, workLogSheetName); readRange := fmt.Sprintf("%s!A:A", workLogSheetName); resp, err := srv.Spreadsheets.Values.Get(spreadsheetID, readRange).Do(); var sheetValues [][]interface{}; if err != nil { log.Printf("Помилка читання дат '%s': %v.", workLogSheetName, err) } else { sheetValues = resp.Values }; rowIndexInValues, _ := findRowByDate(sheetValues, todayStr); actualSheetRowIndex := -1; if rowIndexInValues != -1 { actualSheetRowIndex = rowIndexInValues + 1 }; rowData := []interface{}{todayStr, "Розпочато", startTimeStr, "", ""}; if actualSheetRowIndex != -1 { updateRange := fmt.Sprintf("%s!A%d:E%d", workLogSheetName, actualSheetRowIndex, actualSheetRowIndex); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, updateRange, valueRange).ValueInputOption("USER_ENTERED").Do(); if err != nil { return fmt.Errorf("помилка оновлення запису старту: %w", err) }; log.Printf("Рядок %d (%s) оновлено (старт).", actualSheetRowIndex, todayStr) } else { appendRange := fmt.Sprintf("%s!A:E", workLogSheetName); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Append(spreadsheetID, appendRange, valueRange).ValueInputOption("USER_ENTERED").InsertDataOption("INSERT_ROWS").Do(); if err != nil { return fmt.Errorf("помилка додавання запису старту: %w", err) }; log.Printf("Новий рядок для %s додано (старт).", todayStr) }; return nil
}
...
func LogWorkStop(srv *sheets.Service, spreadsheetID string, workLogSheetName string, chatID int64, endTime time.Time) (time.Duration, error) {
	if workLogSheetName == "" { workLogSheetName = workLogSheetNameDefault }; localEndTime := endTime.In(KyivLocation); todayStr := localEndTime.Format("2006-01-02"); endTimeStr := localEndTime.Format("15:04:05"); log.Printf("Лог завершення: ChatID=%d, Date=%s, Time=%s, Sheet=%s", chatID, todayStr, endTimeStr, workLogSheetName); zeroDuration := time.Duration(0); readRange := fmt.Sprintf("%s!A:C", workLogSheetName); resp, err := srv.Spreadsheets.Values.Get(spreadsheetID, readRange).Do(); if err != nil { return zeroDuration, fmt.Errorf("не прочитано дані для стоп: %w", err) }; rowIndexInValues, rowDataFromFind := findRowByDate(resp.Values, todayStr); actualSheetRowIndex := -1; if rowIndexInValues != -1 { actualSheetRowIndex = rowIndexInValues + 1 }; if actualSheetRowIndex == -1 { return zeroDuration, fmt.Errorf("не знайдено запис про старт") }; var startTimeInKyiv time.Time; var duration time.Duration = zeroDuration; var currentStatus string = "Невідомо"; if len(rowDataFromFind) >= 3 { startTimeSheetStr := fmt.Sprintf("%v", rowDataFromFind[2]); currentStatus = fmt.Sprintf("%v", rowDataFromFind[1]); parsedStartTime, errTime := time.ParseInLocation("15:04:05", startTimeSheetStr, KyivLocation); if errTime == nil { year, month, day := localEndTime.Date(); startTimeInKyiv = time.Date(year, month, day, parsedStartTime.Hour(), parsedStartTime.Minute(), parsedStartTime.Second(), 0, KyivLocation); if (currentStatus == "Розпочато" || currentStatus == "Робочий") && localEndTime.After(startTimeInKyiv) { duration = localEndTime.Sub(startTimeInKyiv) } else { log.Printf("Статус не 'Розпочато' або час некор. для ChatID %d (%s).", chatID, currentStatus) } } else { log.Printf("Не розпарсено час початку '%s': %v", startTimeSheetStr, errTime) } } else { log.Printf("Мало даних у рядку %d.", actualSheetRowIndex) }; durationStr := FormatDuration(duration); newStatus := "Завершено"; updateRangeDE := fmt.Sprintf("%s!D%d:E%d", workLogSheetName, actualSheetRowIndex, actualSheetRowIndex); valueRangeDE := &sheets.ValueRange{Values: [][]interface{}{{endTimeStr, durationStr}}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, updateRangeDE, valueRangeDE).ValueInputOption("USER_ENTERED").Do(); if err != nil { log.Printf("Помилка оновлення D:E рядка %d: %v", actualSheetRowIndex, err) }; statusUpdateRange := fmt.Sprintf("%s!B%d", workLogSheetName, actualSheetRowIndex); statusValueRange := &sheets.ValueRange{Values: [][]interface{}{{newStatus}}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, statusUpdateRange, statusValueRange).ValueInputOption("USER_ENTERED").Do(); if err != nil { return zeroDuration, fmt.Errorf("не оновлено запис стоп: %w", err) }; log.Printf("Рядок %d (%s) оновлено (завершено, %s).", actualSheetRowIndex, todayStr, durationStr); return duration, nil
}

func LogDayOff(srv *sheets.Service, spreadsheetID string, workLogSheetName string, chatID int64, dateToLog time.Time) error {
	if workLogSheetName == "" { workLogSheetName = workLogSheetNameDefault }; localDate := dateToLog.In(KyivLocation); dateStr := localDate.Format("2006-01-02"); log.Printf("Лог вихідного: ChatID=%d, Date=%s, Sheet=%s", chatID, dateStr, workLogSheetName); readRange := fmt.Sprintf("%s!A:A", workLogSheetName); resp, err := srv.Spreadsheets.Values.Get(spreadsheetID, readRange).Do(); var sheetValues [][]interface{}; if err != nil { log.Printf("Помилка читання дат '%s': %v.", workLogSheetName, err) } else { sheetValues = resp.Values }; rowIndexInValues, _ := findRowByDate(sheetValues, dateStr); actualSheetRowIndex := -1; if rowIndexInValues != -1 { actualSheetRowIndex = rowIndexInValues + 1 }; rowData := []interface{}{dateStr, "Вихідний", "", "", ""}; if actualSheetRowIndex != -1 { updateRange := fmt.Sprintf("%s!A%d:E%d", workLogSheetName, actualSheetRowIndex, actualSheetRowIndex); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, updateRange, valueRange).ValueInputOption("USER_ENTERED").Do(); if err != nil { return fmt.Errorf("не оновлено запис вихідного: %w", err) }; log.Printf("Рядок %d (%s) оновлено (вихідний).", actualSheetRowIndex, dateStr) } else { appendRange := fmt.Sprintf("%s!A:E", workLogSheetName); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Append(spreadsheetID, appendRange, valueRange).ValueInputOption("USER_ENTERED").InsertDataOption("INSERT_ROWS").Do(); if err != nil { return fmt.Errorf("не додано запис вихідного: %w", err) }; log.Printf("Новий рядок для %s додано (вихідний).", dateStr) }; return nil
}

func GetActiveGoalFromSheet(srv *sheets.Service, spreadsheetID string, goalsSheetName string, chatID int64) (FinancialGoalData, bool, error) { /* ... код без змін ... */ return FinancialGoalData{}, false, nil }
func CountWorkingDaysInRange(srv *sheets.Service, spreadsheetID string, workLogSheetName string, startDate, endDate time.Time) (int, error) { /* ... код без змін ... */ return 0, nil }

// Я прибрав функцію getMotivation звідси, оскільки вона викликається лише з telegram/report.go
// і не потрібна для інших функцій цього пакета sheets.
// Якщо вона знадобиться десь ще, її можна зробити публічною в пакеті motivation.
