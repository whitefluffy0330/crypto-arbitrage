...
// findRowByDate знаходить 0-based індекс рядка та сам рядок за датою. (ВИЗНАЧЕННЯ)
...
func LogWorkStart(srv *sheets.Service, spreadsheetID string, workLogSheetName string, chatID int64, startTime time.Time) error {
	...
	rowData := []interface{}{todayStr, "Розпочато", startTimeStr, "", ""}; if actualSheetRowIndex != -1 { updateRange := fmt.Sprintf("%s!A%d:E%d", workLogSheetName, actualSheetRowIndex, actualSheetRowIndex); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Update(spreadsheetID, updateRange, valueRange).ValueInputOption("USER_ENTERED").Do(); if err != nil { return fmt.Errorf("помилка оновлення старту: %w", err) }; log.Printf("Рядок %d (%s) оновлено (старт).", actualSheetRowIndex, todayStr) } else { appendRange := fmt.Sprintf("%s!A:E", workLogSheetName); valueRange := &sheets.ValueRange{Values: [][]interface{}{rowData}}; _, err = srv.Spreadsheets.Values.Append(spreadsheetID, appendRange, valueRange).ValueInputOption("USER_ENTERED").InsertDataOption("INSERT_ROWS").Do(); if err != nil { return fmt.Errorf("помилка додавання старту: %w", err) }; log.Printf("Новий рядок для %s додано (старт).", todayStr) }; return nil
}
