...
func GenerateProgressReport(srv *sheets.Service, spreadsheetID string, reportSheetNameAndRange string) (SheetRowData, error) {
	...
	if len(parts) == 2 && parts[0] != "" && parts[1] != "" { 
		sheetName = parts[0]
		// Використовуємо діапазон A2:E для читання даних
...
// LogWorkStart знаходить/додає рядок для сьогодні та записує час старту
...
// LogWorkStop знаходить рядок для сьогодні, записує час кінця і тривалість
...
// LogDayOff знаходить/додає рядок для дати та ставить статус "Вихідний"
