...
import (
	// "context" // Не використовується
	"fmt"
	"log"
	"strconv"
	"strings" // <<< ПОВЕРНУЛИ ЦЕЙ ІМПОРТ!
	"time"

	// "google.golang.org/api/option" // Не використовується
...
func init() {
	loc, err := time.LoadLocation("Europe/Kyiv"); if err != nil { log.Printf("Критична помилка: не вдалося завантажити 'Europe/Kyiv': %v.", err); KyivLocation = time.UTC } else { KyivLocation = loc; log.Println("Часову зону Europe/Kyiv успішно завантажено (пакет sheets).") }
}
...
func NewService(credentialsJSON []byte) (*sheets.Service, error) { return nil, fmt.Errorf("функція NewService не реалізована або не використовується") }
...
func GenerateProgressReport(srv *sheets.Service, spreadsheetID string, reportSheetNameAndRange string) (SheetRowData, error) {
	...
	sheetName := "Звіт"; sheetRangeForRead := "A2:E" // Значення за замовчуванням
	
	// Парсимо назву аркуша та діапазон з конфігурації
	parts := strings.Split(reportSheetNameAndRange, "!") // <<< ВИКОРИСТАННЯ strings.Split
	if len(parts) == 2 && parts[0] != "" && parts[1] != "" {
		sheetName = parts[0]
		// Формуємо діапазон для читання (A2:E) на вказаному аркуші
		...
		log.Printf("ПОПЕРЕДЖЕННЯ: Некоректний формат reportSheetNameAndRange ('%s'). Використовується діапазон за замовчуванням: %s", reportSheetNameAndRange, sheetRangeForRead)
	}
	...
	if len(resp.Values) < 1 { return data, fmt.Errorf("немає даних у діапазоні %s", sheetRangeForRead) }
	...
	if len(row) > 1 { incomeStr := fmt.Sprintf("%v", row[1]); data.Income, err = strconv.ParseFloat(incomeStr, 64); if err != nil { return data, fmt.Errorf("некоректний Дохід ('%s') в '%s'", incomeStr, sheetName) } } else { return data, fmt.Errorf("відсутній Дохід в '%s' (B)", sheetName) }
	if len(row) > 2 { sheetGoalStr := fmt.Sprintf("%v", row[2]); data.SheetGoal, err = strconv.ParseFloat(sheetGoalStr, 64); if err != nil { return data, fmt.Errorf("некоректна Мета ('%s') в '%s'", sheetGoalStr, sheetName) } } else { return data, fmt.Errorf("відсутня Мета в '%s' (C)", sheetName) }
...
// ... (решта функцій: AddGoalToSheet, UpdateGoalStatusInSheet, LogWorkStart, LogWorkStop, LogDayOff, GetActiveGoalFromSheet, CountWorkingDaysInRange залишаються без змін з відповіді #143) ...
